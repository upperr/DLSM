from layers import FullConnection, GraphConvolution, GraphConvolutionSparse, LSMDecoder
...
class DLSM(Model):
    def __init__(self, placeholders, num_features, num_nodes, features_nonzero, mc_samples = 1, **kwargs):
        ...
        self.S = mc_samples # number of MC samples
    ...
    def _build(self):
        ...
        for k in range(self.S):    
            ...
            if FLAGS.directed == 1:
                self.reconstructions, self.theta_decoder = output_layer([self.theta[-1][0], self.theta[-1][3], self.theta[-1][4]])
            else:
                self.reconstructions, self.theta_decoder = output_layer([self.theta[-1][0], self.theta[-1][3]])
        ...
        for idx, encoder_layer in enumerate(self.encoder_layers):
            ...
            if idx == 0:
                ...
                gc_alpha_gam = GraphConvolution(input_dim = self.encoder_layers[idx - 1],
                                     ...
                                     name = "conv_weight_alpha_gam_" + str(idx),
                                     logging = self.logging)
                h_alpha_gam = gc_alpha_gam(self.h[-1][2])
                ...
                if FLAGS.directed == 1:
                    gc_alpha_del = GraphConvolution(input_dim = self.encoder_layers[idx - 1],
                                     ...
                                     name = "conv_weight_alpha_del_" + str(idx),
                                     logging = self.logging)
                    h_alpha_del = gc_alpha_del(self.h[-1][3])
                    
                    self.h.append([h_mean, h_std, h_alpha_gam, h_alpha_del])
                    
                else:
                    self.h.append([h_mean, h_std, h_alpha_gam])
                ...
                alpha_gam_layer = FullConnection(input_dim = encoder_layer, output_dim = self.decoder_layers[idx - 1], act = lambda x: tf.nn.softplus(x))
                alpha_gam = alpha_gam_layer(self.h[-1][2])
            
                if FLAGS.directed == 1:
                    alpha_del_layer = FullConnection(input_dim = encoder_layer, output_dim = self.decoder_layers[idx - 1], act = lambda x: tf.nn.softplus(x))
                    alpha_del = alpha_del_layer(self.h[-1][3])
            
                    self.posterior_theta_param.append([z_mean, z_std, alpha_gam, alpha_del])
                else:
                    self.posterior_theta_param.append([z_mean, z_std, alpha_gam])
        ...
        for k in range(self.S):    
            ...
            for idx, decoder_layer in enumerate(self.decoder_layers): # l = 1, 2, ..., L-1
                ...
                if idx == 0:
                    ...
                    alpha_gam_layer = FullConnection(input_dim = self.encoder_layers[-1], output_dim = decoder_layer, act = lambda x: tf.nn.relu(x))
                    self.posterior_theta_param[idx][2] += alpha_gam_layer(self.h[-1][2])
                    
                    if FLAGS.directed == 1:
                        alpha_del_layer = FullConnection(input_dim = self.encoder_layers[-1], output_dim = decoder_layer, act = lambda x: tf.nn.relu(x))
                        self.posterior_theta_param[idx][3] += alpha_del_layer(self.h[-1][3])
                    ...
                    alpha_gam_layer = FullConnection(input_dim = self.decoder_layers[idx - 1], output_dim = decoder_layer, act = lambda x: tf.nn.relu(x))
                    self.posterior_theta_param[idx][2] += alpha_gam_layer(self.theta[idx - 1][1])
                    
                    if FLAGS.directed == 1:
                        alpha_del_layer = FullConnection(input_dim = self.decoder_layers[idx - 1], output_dim = decoder_layer, act = lambda x: tf.nn.relu(x))
                        self.posterior_theta_param[idx][3] += alpha_del_layer(self.theta[idx - 1][2])
                ...
                alpha_gam_prior = tf.constant(1. / decoder_layer, tf.float32)
                # composing Gamma distribution (with same beta) as Dirichlet distribution
                gamma = sample_gamma(self.posterior_theta_param[idx][2], tf.constant(FLAGS.beta, tf.float32))
                gamma = gamma / (tf.reduce_sum(gamma, axis = 0) + SMALL)
            
                if FLAGS.directed == 1:
                    alpha_del_prior = tf.constant(1. / decoder_layer, tf.float32)
                    # composing Gamma distribution (with same beta) as Dirichlet distribution
                    delta = sample_gamma(self.posterior_theta_param[idx][3], tf.constant(FLAGS.beta, tf.float32))
                    delta = delta / (tf.reduce_sum(delta, axis = 0) + SMALL)
                    
                    self.prior_theta_param.append([alpha_gam_prior, alpha_del_prior])
                    self.theta.append([z, gamma, delta])
                else:
                    self.prior_theta_param.append([alpha_gam_prior])
                    self.theta.append([z, gamma])
