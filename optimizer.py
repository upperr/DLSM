...
class Optimizer(object):
    def __init__(self, labels, model, epoch, num_nodes, pos_weight, norm, edges_for_loss, model_str):
        ...
        for s in range(model.S):
            ...
            neg_ll = self.binary_weighted_crossentropy(preds, labels, pos_weight)
        ...
        for k in range(model.S):
            ...
            for idx in range(model.num_decoder_layers):
                ...
                if model_str == 'dlsm':
                    ...
                    alpha_gam_posterior = model.posterior_theta_param_list[k][idx][2]
                    self.kl_alpha_gam = kl_gamma(alpha_gam_posterior, alpha_gam_prior) / num_nodes
                    
                    if FLAGS.directed == 1:
                        alpha_del_prior = model.prior_theta_param_list[k][idx][1]
                        alpha_del_posterior = model.posterior_theta_param_list[k][idx][3]
                        self.kl_alpha_del = kl_gamma(alpha_del_posterior, alpha_del_prior) / num_nodes
                    else:
                        self.kl_alpha_del = 0.
